...
export const handler = async function (event, context) {
	...
	const pathMatch = event.path.match(
		/\bgist\/(?<gistId>[\da-f]+)(?:\/(?<gistRev>[\da-f]+))?/i,
	);
	...
	const { gistId, gistRev = "" } = pathMatch.groups;
	...
	try {
		...
		// Format CSS
		const cssContent =
			css && css.indexOf("{") === -1 ? `html{${css}}` : css || "";

		// Determine if prefix-free should be used
		const usePrefixFree =
			(!settingsObj.version && !settingsObj.prefixfree) ||
			settingsObj.prefixfree ||
			settingsObj.settings?.prefixfree;
